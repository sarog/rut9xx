...
m = Map("wireless", translate("Wifi"), translate("Here you can configure your wireless network."))
m:chain("network")
...
function link.cfgvalue(self, section)
	...
	return wifidata[ifname] and wifidata[ifname][translate("Link Quality")] or "-"
end

essid = s:option(DummyValue, "ssid", translate("ESSID"))

bssid = s:option(DummyValue, "_bsiid", translate("BSSID"))
...
encryption = s:option(DummyValue, "encryption", translate("Encr."))
...
t2 = m:section(Table, {}, translate("WLAN-Scan"), translate("Wifi networks in your local environment"))
...
essid = t2:option(DummyValue, "ESSID", translate("ESSID"))
...
function chan.cfgvalue(self, section)
	return self.map:get(section, translate("Channel"))
	    or self.map:get(section, translate("Frequency"))
	    or "-"
end

t2:option(DummyValue, "Encryption key", translate("Encr."))
...
s:option(Value, "ssid", translate("Network Name (ESSID)"))
...
if hwtype == "atheros" or hwtype == "broadcom" then
	...
	hide = s:option(Flag, "hidden", translate("Hide ESSID"))
	hide.rmempty = true
