...
TEST_GROUP(unit_twWs_Disconnect);

TEST_SETUP(unit_twWs_Disconnect){
	eatLogs();
	twApi_Initialize(TW_HOST, TW_PORT, TW_URI, TW_APP_KEY, NULL, MESSAGE_CHUNK_SIZE, MESSAGE_CHUNK_SIZE, TRUE);
}

TEST_TEAR_DOWN(unit_twWs_Disconnect){
	twStubs_Reset ();
	CleanupCSDK1196Payload();
	twApi_Delete();
}
TEST_GROUP_RUNNER(unit_twWs_Disconnect) {
	RUN_TEST_CASE(unit_twWs_Disconnect, test_disconnect_message_buffer);
}

/**
* Test Plan: Ensure that lengthy disconnect reasons do not risk buffer overflow
*/
TEST(unit_twWs_Disconnect, test_disconnect_message_buffer) {
	twWs ws;
	char frameBuffer[1024];

	InitializeFakeWebsocket(&ws, frameBuffer);

	TEST_ASSERT_EQUAL (TW_OK, twWs_Disconnect(tw_api->mh->ws, NORMAL_CLOSE, "A reason of this length should be fine."));
	TEST_ASSERT_EQUAL (TW_OK, twWs_Disconnect(tw_api->mh->ws, NORMAL_CLOSE, "A reason of this length should trunc the period ."));
	TEST_ASSERT_EQUAL (TW_OK, twWs_Disconnect(tw_api->mh->ws, NORMAL_CLOSE, "A reason of this length would overflow without protection."));
