...
m = Map (modulename, translate("Advanced Settings"),
         translate("This section contains settings that do not need to be changed under normal circumstances. In addition, here you can configure your system for use with remote SIP devices, and resolve call quality issues by enabling the insertion of QoS rules."))
...
s:tab("remote_usage", translate("Remote Usage"),
      translatef("You can use your SIP devices/softphones with this system from a remote location as well, as long as your Internet Service Provider gives you a public IP. You will be able to call other local users for free (e.g. other Analog Telephone Adapters (ATAs)) and use your VoIP providers to make calls as if you were local to the PBX. After configuring this tab, go back to where users are configured and see the new Server and Port setting you need to configure the remote SIP devices with. Please note that if this PBX is not running on your router/gateway, you will need to configure port forwarding (NAT) on your router/gateway. Please forward the ports below (SIP port and RTP range) to the IP address of the device running this PBX."))

s:tab("qos",  translate("QoS Settings"), 
      translate("If you experience jittery or high latency audio during heavy downloads, you may want to enable QoS. QoS prioritizes traffic to and from your network for specified ports and IP addresses, resulting in better latency and throughput for sound in our case. If enabled below, a QoS rule for this service will be configured by the PBX automatically, but you must visit the QoS configuration page (Network->QoS) to configure other critical QoS settings like Download and Upload speed."))

ringtime = s:taboption("general", Value, "ringtime", translate("Number of Seconds to Ring"),
                 translate("Set the number of seconds to ring users upon incoming calls before hanging up or going to voicemail, if the voicemail is installed and enabled."))
ringtime.default = 30

ua = s:taboption("general", Value, "useragent", translate("User Agent String"),
                 translate("This is the name that the VoIP server will use to identify itself when registering to VoIP (SIP) providers. Some providers require this to a specific string matching a hardware SIP device."))
ua.default = appname

h = s:taboption("remote_usage", Value, "externhost", translate("Domain/IP Address/Dynamic Domain"),
                translate("You can enter your domain name, external IP address, or dynamic domain name here Please keep in mind that if your IP address is dynamic and it changes your configuration will become invalid. Hence, it's recommended to set up Dynamic DNS in this case."))
h.datatype = "hostname"

p = s:taboption("remote_usage", Value, "bindport", translate("External SIP Port"),
                translate("Pick a random port number between 6500 and 9500 for the service to listen on. Do not pick the standard 5060, because it is often subject to brute-force attacks. When finished, (1) click \"Save and Apply\", and (2) click the \"Restart VoIP Service\" button above. Finally, (3) look in the \"SIP Device/Softphone Accounts\" section for updated Server and Port settings for your SIP Devices/Softphones."))
p.datatype = "port"

p = s:taboption("remote_usage", Value, "rtpstart", translate("RTP Port Range Start"),
                translate("RTP traffic carries actual voice packets. This is the start of the port range that will be used for setting up RTP communication. It's usually OK to leave this at the default value."))
p.datatype = "port"
