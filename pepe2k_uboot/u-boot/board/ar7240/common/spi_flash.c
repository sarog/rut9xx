...
extern void led_toggle_invert(void);
				...
				u32 s_last)
{
	...
	for (i = s_first; i <= s_last; i++) {
		...
		if (!(gpio & GPIO_LED_ETH0)) {
			gpio = GPIO_LED_ETH0;
		} else if (!(gpio & GPIO_LED_ETH1)){
			gpio = GPIO_LED_ETH0 | GPIO_LED_ETH1;
		} else if (!(gpio & GPIO_LED_ETH2)) {
			gpio = GPIO_LED_ETH0 | GPIO_LED_ETH1 | GPIO_LED_ETH2;
		} else if (!(gpio & GPIO_LED_ETH3)) {
			gpio = GPIO_LED_ETH0 | GPIO_LED_ETH1 | GPIO_LED_ETH2 | GPIO_LED_ETH3;
		} else {
			gpio = 1;
		}
...
u32 write_buff(flash_info_t *info, uchar *source, ulong addr, ulong len)
{
	...
	while (total < len) {
		...
		if(i == 256){
 			...
 			gpio = ar7240_reg_rd(QCA_GPIO_OUT_REG);
			if (gpio & GPIO_LED_ETH0) {
				gpio = GPIO_LED_ETH1 | GPIO_LED_ETH2 | GPIO_LED_ETH3;
			} else if (gpio & GPIO_LED_ETH1) {
				gpio = GPIO_LED_ETH2 | GPIO_LED_ETH3;
			} else if (gpio & GPIO_LED_ETH2) {
				gpio = GPIO_LED_ETH3;
			} else if (gpio & GPIO_LED_ETH3) {
				gpio = 1;
