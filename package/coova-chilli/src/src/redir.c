...
int redir_reply(struct redir_t *redir, struct redir_socket_t *sock,
  ...
  case REDIR_CHALLENGE:
    ...
    break;
  case REDIR_BLOCKED:
    sprintf(buff, "failed&reason=blocked&expiration=%lld",
            (long long) conn->s_params.restriction_expiration);
    resp = buff;
	break;
...
time_t get_expiration_date(struct session_params *params){
    ...
    switch (params->data_interval){
        case INTERVAL_DAY:
            if (params->start_point < time_now->tm_hour)
                time_now->tm_mday++;

            time_now->tm_hour = params->start_point;
            break;
        case INTERVAL_WEEK:
            start_wday = params->start_point == 0 ? 7 : params->start_point;
            wday = time_now->tm_wday == 0 ? 7 : time_now->tm_wday;

            if (time_now->tm_wday != params->start_point){
                if (start_wday < wday)
                    time_now->tm_mday += (7 - (wday - start_wday));
                else
                    time_now->tm_mday += (start_wday - wday);
            }

            time_now->tm_hour = 0;
            break;
        case INTERVAL_MONTH:
            if (params->start_point < time_now->tm_mday)
                time_now->tm_mon++;

            time_now->tm_mday = params->start_point;
            time_now->tm_hour = 0;
            break;
        default:
            break;
...
int check_user(const char * user, struct session_params *params, char *download, char *upload){
	...
	if (!_options.dhcpif || !user)
		return 0;

	if (params->maxinputoctets || params->maxoutputoctets)
        sprintf(script, "/usr/sbin/statistics check %s %s %d %d 2>&1", user, _options.dhcpif,
                params->data_interval, params->start_point);
	else if (_options.defmaxinputoctets || _options.defmaxoutputoctets)
        	sprintf(script, "/usr/sbin/statistics check %s %s %d %d 2>&1", user, _options.dhcpif,
                	_options.defdatainterval , _options.defstartpoint);
	else
		return 0;
...
int check_mac(uint8_t *mac, char *download, char *upload, char *time){
	...
	if (mac)
		sprintf(script, "/usr/sbin/statistics check_mac_all %.2X-%.2X-%.2X-%.2X-%.2X-%.2X %s %d %d 2>&1",
                mac[0], mac[1], mac[2], mac[3], mac[4], mac[5], _options.dhcpif, _options.defdatainterval,
                _options.defstartpoint);
	else
		return 0;
		...
		if (match == 1){
			...
			*p_data_interval=0;
			...
			if (check_user(conn->s_state.redir.username, &conn->s_params, download, upload)){
				if (conn->s_params.maxinputoctets && atoi(download) > conn->s_params.maxinputoctets)
					match = 2;
				else if (conn->s_params.maxoutputoctets && atoi(upload) > conn->s_params.maxoutputoctets)
					match = 2;
			}
			else {
  ...
  */
  ...
  case REDIR_LOGIN: {
    ...
    if (match) {
        ...
        if (conn.response == REDIR_BLOCKED)
            conn.s_params.restriction_expiration = get_expiration_date(&conn.s_params);
    }
    else {
