...
s = m:section(TypedSection, "callthrough_numbers", translate("Call-through Numbers"),
        translate("Designate numbers that are allowed to call through this system and which user's privileges it will have."))
s.anonymous = true
s.addremove = true
...
user = s:option(Value, "defaultuser",  translate("User Name"),
         translate("The number(s) specified above will be able to dial out with this user's providers. Invalid usernames, including users not enabled for outgoing calls, are dropped silently. Please verify that the entry was accepted."))
...
pwd = s:option(Value, "pin", translate("PIN"),
               translate("Your PIN disappears when saved for your protection. It will be changed only when you enter a value different from the saved one. Leaving the PIN empty is possible, but please beware of the security implications."))
pwd.password = true
pwd.rmempty = false
...
s = m:section(NamedSection, "blacklisting", "call_routing", translate("Blacklisted Numbers"),
              translate("Enter phone numbers that you want to decline calls from automatically. You should probably omit the country code and any leading zeroes, but please experiment to make sure you are blocking numbers from your desired area successfully."))
s.anonymous = true
