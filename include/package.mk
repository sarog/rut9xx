...
define GenInfo
	printf "%s," \
		"$(PKG_NAME)" \
		"$(if $(PKG_VERSION),$(PKG_VERSION)$(if $(PKG_RELEASE),-$(PKG_RELEASE)),$(if $(PKG_SOURCE_DATE),$(PKG_SOURCE_DATE),$(if $(PKG_RELEASE),$(PKG_RELEASE),-)))" \
		"$(if $(PKG_LICENSE),$(PKG_LICENSE),-)" \
		"$(if $(PKG_SOURCE_URL),$(call Download/ParseURL,$(firstword $(PKG_SOURCE_URL))),-)" \
			>> "$(GENINFO_FILE)"
	printf "\n" >> "$(GENINFO_FILE)"
endef
