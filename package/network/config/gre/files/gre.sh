...
gre_generic_setup() {
	...
	local remote="$4"
	local link="$5"
	local mtu ttl tos zone ikey okey icsum ocsum iseqno oseqno multicast enabled
	json_get_vars mtu ttl tos zone ikey okey icsum ocsum iseqno oseqno multicast enabled
...
gre_setup() {
	...
	[ -z "$ipaddr" ] && {
		...
		if [ -n "$wanif" ] && ! network_get_device wanif "$tunlink"; then
			proto_notify_error "$cfg" "NO_WAN_LINK"
			exit
		fi

		if [ -z "$wanif" ] && ! network_find_wan wanif; then
			proto_notify_error "$cfg" "NO_WAN_LINK_IFACE"
			exit
		fi

		if ! network_get_l3_ipaddr ipaddr "$wanif"; then
...
proto_gre_init_config() {
	...
	proto_config_add_boolean "enabled"
}
