				...
				u32 s_last)
{
	...
	if (info->can_be_locked) {
		printf("Micron Authenta flash, DYB unlock may be executed");
		printf("\n\n");
	}
	printf("Erasing: ");
	...
	for (i = s_first; i <= s_last; i++) {
		if (info->can_be_locked) {
			if (qca_sf_sect_is_locked(info->bank, i * info->sector_size,
									 info->sector_size, SPI_FLASH_DYB_RD)) {
				qca_sf_sect_unlock(info->bank, i * info->sector_size,
								   info->sector_size, SPI_FLASH_DYB_WR);
			}
		}
