...
static void fill_header(char *buf, int len)
{
	...
	fclose(fileptr);
	//Funkcija skirta pataisyti 3 versijos fw del pasikeitimo i 4 ir keepsetings islaikymo
	minor_len = strlen(fw_name);
	minor_nr = &fw_name[minor_len-1];
	while (*minor_nr != '.'){
		minor_nr--;
	}
	minor_nr++;
	sprintf(minor_nr, "%d", atoi(minor_nr)+1000);
	//-------------------------------------------------------------------------
...
int main(int argc, char *argv[])
{
	...
	while ( 1 ) {
		...
		c = getopt(argc, argv, "a:B:H:E:F:L:V:N:S:W:ci:k:r:R:o:xX:hsjv:");
		if (c == -1)
			break;
