...
-------------------- View --------------------
f = SimpleForm("siitwizward", translate("SIIT-Wizzard"),
 translate("This wizzard helps to setup SIIT (IPv4-over-IPv6) translation according to RFC2765."))

f:field(DummyValue, "info_ula", translate("Mesh ULA address")).value = ula:string()

f:field(DummyValue, "ipv4_pool", translate("IPv4 allocation pool")).value =
	"%s (%i hosts)" %{ gv4_net:string(), 2 ^ ( 32 - gv4_net:prefix() ) - 2 }

f:field(DummyValue, "ipv4_size", translate("IPv4 LAN network prefix")).value =
	"%i bit (%i hosts)" %{ ipv4_netsz, 2 ^ ( 32 - ipv4_netsz ) - 2 }

mode = f:field(ListValue, "mode", translate("Operation mode"))
mode:value("client", translate("Client"))
mode:value("gateway", translate("Gateway)"))

dev = f:field(ListValue, "device", translate("Wireless device"))
uci:foreach("wireless", "wifi-device",
	function(section)
		dev:value(section[".name"])
	end)

lanip = f:field(Value, "ipaddr", translate("LAN IPv4 subnet"))
	...
	if net then
		if gv4_net:contains(net) then
			if not net:minhost():equal(net:host()) then
				...
				f.errmessage = translate("IPv4 address is not the first host of subnet, expected ") .. net:minhost():string()
			end
		else
			self.error = { [section] = true }
			f.errmessage = translate("IPv4 address is not within the allocation pool")
		end
	else
		self.error = { [section] = true }
		f.errmessage = translate("Invalid IPv4 address given")
	end

	return val
end

dns = f:field(Value, "dns", translate("DNS server for LAN clients"))
dns.value = "141.1.1.1"
