...
function action_status()
	...
	if data then
		...
		local profile = uci:get("profiles", "general", "profile")
		local required_profile = luci.http.formvalue("required_profile")
		if required_profile then
			if required_profile ~= profile then
				luci.http.write("Changing profile...")
				return
			end
		end
