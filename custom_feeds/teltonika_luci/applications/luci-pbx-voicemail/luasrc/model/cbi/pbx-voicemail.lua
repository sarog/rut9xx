...
m = Map (modulename, translate("Voicemail Setup"),
         translate("Here you can configure a global voicemail for this PBX. Since this system is intended to run on embedded systems like routers, there is no local storage of voicemail - it must be sent out by email. Therefore you need to configure an outgoing mail (SMTP) server (for example your ISP's, Google's, or Yahoo's SMTP server), and provide a list of addresses that receive recorded voicemail."))
...
s = m:section(NamedSection, "global_voicemail", "voicemail", translate("Global Voicemail Setup"),
              translate("When you enable voicemail, you will have the opportunity to specify email addresses that receive recorded voicemail. You must also set up an SMTP server below."))
s.anonymous = true
...
savepath = s:option(Value, "global_save_path", translate("Local Storage Directory"),
                    translate("You can also retain copies of voicemail messages on the device running your PBX. The path specified here will be created if it doesn't exist. Beware of limited space on embedded devices like routers, and enable this option only if you know what you are doing."))
...
if     nixio.fs.access("/etc/pbx-voicemail/greeting.WAV")   then
   ...
   m1.default = translate("NOTE: Found a voicemail greeting. To check or change your voicemail greeting, dial *789 and the system will play your current message. You have 5 seconds to hangup, otherwise a new recording will begin and your old message will be overwritten. Hang up or press # to stop recording. When you press #, the system will play back the new recording.")
else
   m1 = s:option(DummyValue, "_m1")
   m1:depends("enabled", "yes")
   m1.default = translate("WARNING: Could not find voicemail greeting. Callers will hear only a beep before recording starts. To record a greeting, dial *789 and record a greeting after the beep. Hang up or press # to stop recording. When # is pressed the system will play back the recording.")
end


----------------------------------------------------------------------------------------------------
s = m:section(NamedSection, "voicemail_smtp", "voicemail", translate("Outgoing mail (SMTP) Server"),
              translate("In order for this PBX to send emails containing voicemail recordings, you need to set up an SMTP server here. Your ISP usually provides an SMTP server for that purpose. You can also set up a third party SMTP server such as the one provided by Google or Yahoo."))
s.anonymous = true
...
pwd = s:option(Value, "smtp_password", translate("SMTP Password"),
               translate("Your real SMTP password is not shown for your protection. It will be changed only when you change the value in this box."))
pwd.password = true
pwd:depends("smtp_auth", "on")
                            
-- We skip reading off the saved value and return nothing.
function pwd.cfgvalue(self, section)
   return translate("Password Not Displayed")
end
...
function pwd.write(self, section, value)
   ...
   if value == translate("Password Not Displayed") then value = "" end
...
function sts.cfgvalue(self, section)
   ...
   if log == nil or log == "" then
      log = translate("No errors or messages reported.")
   end
   return log
end
