...
function format_indices(string, indices)
   if indices == nil then
      return translate("Error: No indices to format specified.\n" )
   end
...
function sts.cfgvalue(self, section)
   ...
   elseif server == "freeswitch" then
      return translate("Freeswitch is not supported yet.\n")
   else
      return translate("Neither Asterisk nor FreeSwitch discovered, please install Asterisk, as Freeswitch is not supported yet.\n")
   end
end
