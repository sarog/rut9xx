...
function sysupgrade(r)
	...
	local deviceId = r:value(2)
	local url = r:value(3)
	local username = uci:get("iot", "cumulocity", "username")
	local password = uci:get("iot", "cumulocity", "password")

	c8y:send('108,'..deviceId..',EXECUTING')

	result = os.execute("/usr/bin/curl -y 60 -u '"..username.."':'"..password.."' -o /tmp/cud_firmware.bin "..url) / 256
	...
	result = os.execute("/sbin/sysupgrade -T /tmp/cud_firmware.bin") / 256
	if not result or result ~= 0 then
...
function init()
	...
	local INTERVAL = uci:get("iot", "cumulocity", "interval")
	srInfo('*** Stream Init ***')
	...
	MODEM = uci:get("iot", "cumulocity", "modem") or ""
	if MODEM == "" then
...
function mobileDataStream()
	if MODEM and #MODEM > 0 then
		...
		if results.signal ~= "N/A" then signal_value = results.signal else signal_value = "0" end
	else 
		signal_value = "0"
	end
