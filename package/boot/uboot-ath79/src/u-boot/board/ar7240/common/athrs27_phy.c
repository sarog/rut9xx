...
void athrs27_set_power(unsigned int phy_addr, unsigned int power){
	unsigned int reg = power ? ATHR_ADVERTISE_ALL : ATHR_ADVERTISE_PAUSE;

	s27_wr_phy(phy_addr, ATHR_AUTONEG_ADVERT, reg);

	s27_wr_phy(phy_addr, 0x9, 0x0); // do not advertise 1000Mbps mode
	s27_wr_phy(phy_addr, ATHR_PHY_CONTROL, 0x0);
	s27_wr_phy(phy_addr, ATHR_PHY_CONTROL, ATHR_CTRL_AUTONEGOTIATION_ENABLE |
