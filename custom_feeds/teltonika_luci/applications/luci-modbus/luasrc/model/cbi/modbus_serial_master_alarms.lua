...
m = Map("modbus_master_alarms")

local serial_type
m.uci:foreach("modbus_serial_master", "rs232_slave", function(s)
	if s.section_id == alarm_cfg then
		serial_type = "rs232"
	end
end)


if serial_type == "rs232" then
	m.redirect = ds.build_url("admin/services/modbus/modbus_serial_master/rs232/")
else
	m.redirect = ds.build_url("admin/services/modbus/modbus_serial_master/rs485")
end
...
s = m:section(TypedSection, "serial_alarm_" .. alarm_cfg, "Modbus Master Alarms")
s.template  = "cbi/tblsection"
s.addremove = true
s.anonymous = true
s.sortable  = false
s.extedit   = ds.build_url("admin/services/modbus/modbus_serial_master_alarm_details/%s")
...
function s.parse(self, ...)
	...
	if created then
		...
		luci.http.redirect(ds.build_url("admin/services/modbus/modbus_serial_master_alarm_details", created))
	end
end
...
function s.remove(self, section)
	...
	if (section) then
		...
		if (slave_cfg_name) then
			m.uci:foreach("modbus_serial_master", "request_" .. slave_cfg_name, function(s)
	...
	m.uci:save("modbus_serial_master")
	m.uci:commit("modbus_serial_master")
	luci.http.redirect(ds.build_url("admin/services/modbus/modbus_serial_master_alarms/" .. alarm_cfg))
end
