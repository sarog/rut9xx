...
function index()
	...
	entry({"admin", "telephony", "account"}, cbi("luci_ltqtapi/account") , _(translate("Account")), 10)
	entry({"admin", "telephony", "contact"}, cbi("luci_ltqtapi/contact") , _(translate("Contacts")), 20)
...
function tapi_status()
	...
	st.status = translate("Offline");
	if state:get("telephony", "state", "port1", "0") == "0" then
		st.line1 = translate("Idle");
	else
		st.line1 = translate("Calling");
	end
	if state:get("telephony", "state", "port2", "0") == "0" then
		st.line2 = translate("Idle");
	else
		st.line2 = translate("Calling");
	end
