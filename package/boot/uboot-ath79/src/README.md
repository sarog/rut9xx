...
# LED state, on ALL RUTOS devices
1. After power is applied to router, all board LEDs must light up as soon as possible, and stay lit until kernel takes over. This is used during manufacturing, for testing LEDs with CV.
2. During `httpd` cmd, before FW upload begins, if `httpd` was started with Digital Input trigger, LEDs must **stay on constantly**.  
  2.1 During `httpd` cmd, before FW upload begins, if `httpd` was started with reset button or via uboot console, LEDs must **blink**.
___
