...
static USHORT WdsSetQMUXBindMuxDataPort(PQMUX_MSG pMUXMsg, void *arg) {
    ...
    pMUXMsg->BindMuxDataPortReq.ep_type = cpu_to_le32(0x02); //DATA_EP_TYPE_HSUSB
    pMUXMsg->BindMuxDataPortReq.iface_id = cpu_to_le32(0x04);
    pMUXMsg->BindMuxDataPortReq.TLV2Type = 0x11;
    pMUXMsg->BindMuxDataPortReq.TLV2Length = cpu_to_le16(0x01);
    pMUXMsg->BindMuxDataPortReq.MuxId = *((UCHAR *)arg);
...
//Underlying Link Layer Protocol
    ...
    pMUXMsg->SetDataFormatReq.UnderlyingLinkLayerProtocolTlv.Value = cpu_to_le32(0x02);     /* Set Ethernet  mode */
...
//Maximum number of datagrams in a single aggregated packet on downlink
    ...
    pMUXMsg->SetDataFormatReq.DownlinkDataAggregationMaxDatagramsTlv.Value = cpu_to_le32(0x01);
//Maximum size in bytes of a single aggregated packet allowed on downlink
    ...
    pMUXMsg->SetDataFormatReq.DownlinkDataAggregationMaxSizeTlv.Value = cpu_to_le32(2048);
#if 0
//Maximum number of datagrams in a single aggregated packet on downlink
    pMUXMsg->SetDataFormatReq.UplinkDataAggregationMaxDatagramsTlv.TLVType = 0x15; 
    pMUXMsg->SetDataFormatReq.UplinkDataAggregationMaxDatagramsTlv.TLVLength = cpu_to_le16(4);
    pMUXMsg->SetDataFormatReq.UplinkDataAggregationMaxDatagramsTlv.Value = cpu_to_le32(0x01);
//Maximum size in bytes of a single aggregated packet allowed on downlink
    pMUXMsg->SetDataFormatReq.UplinkDataAggregationMaxSizeTlv.TLVType = 0x16; 
    pMUXMsg->SetDataFormatReq.UplinkDataAggregationMaxSizeTlv.TLVLength = cpu_to_le16(4);
    pMUXMsg->SetDataFormatReq.UplinkDataAggregationMaxSizeTlv.Value = cpu_to_le32(2048);

#else
//Peripheral End Point ID
    pMUXMsg->SetDataFormatReq.epTlv.TLVType = 0x17; 
    pMUXMsg->SetDataFormatReq.epTlv.TLVLength = cpu_to_le16(8);
    pMUXMsg->SetDataFormatReq.epTlv.ep_type = cpu_to_le32(0x05); // DATA_EP_TYPE_BAM_DMUX
    pMUXMsg->SetDataFormatReq.epTlv.iface_id = cpu_to_le32(0x08); 
#endif
...
int requestSetEthMode(PROFILE_T *profile) {
    ...
    if (profile->qmapnet_adapter) {
        UCHAR MuxId = (profile->qmapnet_adapter != profile->usbnet_adapter) ? (0x80 + profile->pdp) : 0x81;
        pRequest = ComposeQMUXMsg(QMUX_TYPE_WDS, QMIWDS_BIND_MUX_DATA_PORT_REQ , WdsSetQMUXBindMuxDataPort, (void *)&MuxId);
        QmiThreadSendQMI(pRequest, &pResponse);
        if (pResponse) free(pResponse);
