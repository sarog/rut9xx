...
function index()
	...
	entry(
		...
		cbi("olsr/olsrdhna"), _(translate("HNA Announcements"))
	)

	oplg = entry(
		{"admin", "services", "olsrd", "plugins"},
		cbi("olsr/olsrdplugins"), _(translate("Plugins"))
	)

	odsp = entry(
		{"admin", "services", "olsrd", "display"},
		cbi("olsr/olsrddisplay"), _(translate("Display"))
		)
	...
	uci:foreach("olsrd", translate("LoadPlugin"),
		function (section)
