...
o = s:taboption("general", Value, "proxyAddress", translate("Listen address"),
	translate("The interface on which Polipo will listen. To listen on all interfaces use 0.0.0.0 or :: (IPv6)."))
...
o = s:taboption("general", DynamicList, "allowedClients",
	...
	translate("When listen address is set to 0.0.0.0 or :: (IPv6), you must list clients that are allowed to connect. The format is IP address or network address (192.168.1.123, 192.168.1.0/24, 2001:660:116::/48 (IPv6))"))
...
dns = s:taboption("dns", Value, "dnsNameServer", translate("DNS server address"),
	translate("Set the DNS server address to use, if you want Polipo to use different DNS server than the host system."))
...
l:value("happily",
	translate("Query DNS directly, for unknown hosts fall back to system resolver"))
l:value("reluctantly",
...
o = s:taboption("proxy", Value, "parentProxy",
	...
	translate("Parent proxy address (in host:port format), to which Polipo will forward the requests."))
...
o = s:taboption("proxy", Value, "parentAuthCredentials",
	...
	translate("Basic HTTP authentication supported. Provide username and password in username:password format."))
...
v = s:taboption("logging", Value, "logFile",
	...
	translate("Use of external storage device is recommended, because the log file is written frequently and can grow considerably."))
...
s:taboption("general", Value, "diskCacheRoot", translate("Disk cache location"),
	translate("Location where polipo will cache files permanently. Use of external storage devices is recommended, because the cache can grow considerably. Leave it empty to disable on-disk cache.")).rmempty = true
...
s = m:section(NamedSection, "pmm", "polipo",
	...
	translate("Poor Man's Multiplexing (PMM) is a technique that simulates multiplexing by requesting an instance in multiple segments. It tries to lower the latency caused by the weakness of HTTP protocol. NOTE: some sites may not work with PMM enabled."))

s:option(Value, "pmmSize", translate("PMM segments size (in bytes)"),
	translate("To enable PMM, PMM segment size must be set to some positive value.")).rmempty = true

s:option(Value, "pmmFirstSize", translate("First PMM segment size (in bytes)"),
	translate("Size of the first PMM segment. If not defined, it defaults to twice the PMM segment size.")).rmempty = true
