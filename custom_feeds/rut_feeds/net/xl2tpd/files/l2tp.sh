...
proto_l2tp_setup() {
	...
	local ip serv_addr server host defaultroute enabled

	json_get_vars server defaultroute enabled

		[ "$enabled" != "1" ] && {
		echo "Interface not enabled"
	...
	for ip in $(resolveip -t 5 "$host"); do
		[ "$defaultroute" = "1" ] && ( proto_add_host_dependency "$interface" "$ip" )
		serv_addr=1
	done
	[ -n "$serv_addr" ] || {
...
ip-up-script /lib/netifd/l2tp-up
ipv6-up-script /lib/netifd/l2tp-up
ip-down-script /lib/netifd/l2tp-down
ipv6-down-script /lib/netifd/l2tp-down
