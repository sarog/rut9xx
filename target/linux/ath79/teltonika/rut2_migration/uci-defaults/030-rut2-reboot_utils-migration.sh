...
migrate_ping() {
        ...
        action=$(translate_action "$old_action")
...
migrate_wget() {
        local section="$1"
        local enable action time timeout retry host \
                phone_list message new_action
	...
	uci_set ping_reboot "$section_ping" action "$new_action"
