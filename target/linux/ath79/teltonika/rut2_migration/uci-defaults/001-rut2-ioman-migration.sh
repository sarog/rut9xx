...
# outs
config_get dout1_active_high ioman active_DOUT1_status 1

########################## restore IO states on bootup #########################
config_get dout1_new ioman default_DOUT1_status 0
...
rule_to_ioj_cb() {
	...
	if [ $need_2_input_action_pairs -eq 1 ]; then
		uci_set iojuggler @input[-2] trigger $trigger2
