...
s2 = m:section(NamedSection, zone.sid, "fwd_out",
	...
	translatef("The options below control the forwarding policies between zone (%s) and other zones. <em>Destination zones</em> gets forwarded traffic <strong>originating from %q</strong>.",
		escapeHTML(zone:name()), escapeHTML(zone:name()), escapeHTML(zone:name())))

s2.customDesc = "The options below control the forwarding policies between zone (" .. escapeHTML(zone:name()) .. ") and other zones. <em>Destination zones</em> gets forwarded traffic <strong>originating from " .. escapeHTML(zone:name()) .. "</strong>."

out = s2:option(Value, "out",
	translate("Allow forward to <em>destination zones</em>:"))

out.customTitle = "Allow forward to <em>destination zones</em>:"
out.nocreate = true
out.widget = "checkbox"
