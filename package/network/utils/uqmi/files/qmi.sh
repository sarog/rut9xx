...
proto_qmi_setup() {
	...
	json_get_vars apn pdp device modem pdptype sim delay method mtu dhcp ip4table ip6table dhcpv6 leasetime mac $PROTO_DEFAULT_OPTIONS
...
--password "$password"}")
	...
	[ "$method" = "bridge" ] || [ "$method" = "passthrough" ] && [ -n "$cid_4" ] && {
		setup_bridge_v4 "$net_device"

		#Passthrough
		[ "$method" = "passthrough" ] && {
			iptables -tnat -I postrouting_rule -o "$net_device" -j SNAT --to "$bridge_ipaddr"
