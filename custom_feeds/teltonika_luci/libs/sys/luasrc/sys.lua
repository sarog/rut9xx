	...
	for line in ps do
		...
		if row[k[1]] then
			...
			if dir ~= "/dev/mtdblock" then
				local memoryexpansionfilter = not memoryexpansion or row[k[1]]:find("/dev/sda1") == nil
				if row[k[6]]:find("/mnt/") ~= nil and memoryexpansionfilter == true then
...
function sysinfo()
	...
	local memavailable = tonumber(meminfo:match("MemAvailable:%s*(%d+)"))
	...
	return system, model, memtotal, memcached, membuffers, memfree, bogomips, memavailable
end
