...
static int search(int rp, int j, int s0, int s1, int sign, struct _seg *answer)
{
	...
	for (n=next_up(j); n!=next_up(rp); n=next_up(n)) {
		if (0) DBG("d_ring[%d].s.ss[%d]=%f d_ring[%d].s.ss[%d]=%f\n",
...
int contemplate_data(unsigned int absolute, double skew, double errorbar, int freq)
{
	...
	if (valid==(RING_SIZE)) {
		...
		if (dinit) DBG("found maxseg vs. save_min inconsistency\n");
		...
		if (dinit) DBG("found minseg vs. save_max inconsistency\n");
		if (dinit) inconsistent=1;
		min_imax = c;
		minseg[min_imax] = save_max;

		/* not needed for analysis, but shouldn't hurt either */
		if (g_debug) polygon_point(&save_min);
		} /* !inconsistent */
