...
function cbi_add_networks(field,Type)
	local show =  luci.tools.status.show_mobile()
	local state = uci.cursor_state()
	state:load("network")
	uci.cursor():foreach("network", "interface",
		function (section)
			if section[".name"] ~= "loopback" and section[".name"] ~= "wan6" then
				if section[".name"] == "ppp" then
					if show then
						field:value(section[".name"], section[".name"]:upper())
					end
				elseif Type == "routes" then
					local proto = state:get("network", section[".name"], "proto")
					if proto == "pptp" then
						field:value(section[".name"], "pptp-"..section[".name"]:upper())
					else
						field:value(section[".name"], section[".name"]:upper())
					end
				else
