...
m = Map (modulename, translate("User Accounts"), 
        translate("Here you must configure at least one SIP account, that you will use to register with this service. Use this account either in an Analog Telephony Adapter (ATA), or in a SIP software like CSipSimple, Linphone, or Sipdroid on your smartphone, or Ekiga, Linphone, or X-Lite on your computer. By default, all SIP accounts will ring simultaneously if a call is made to one of your VoIP provider accounts or GV numbers."))
...
if ipaddr == nil or ipaddr == "" then
   ipaddr = translate("(IP address not static)")
end
...
s:option(DummyValue, "ipaddr", translate("Server Setting for Local SIP Devices"),
         translate("Enter this IP (or IP:port) in the Server/Registrar setting of SIP devices you will use ONLY locally and never from a remote location.")).default = ipaddr
...
if externhost ~= nil then
   ...
   s:option(DummyValue, "externhost", translate("Server Setting for Remote SIP Devices"),
            translate("Enter this hostname (or hostname:port) in the Server/Registrar setting of SIP devices you will use from a remote location (they will work locally too).")
                     ).default = externhost
end

if bindport ~= nil then
        s:option(DummyValue, "bindport", translate("Port Setting for SIP Devices"),
        translatef("If setting Server/Registrar to %s or %s does not work for you, try setting it to %s or %s and entering this port number in a separate field that specifies the Server/Registrar port number. Beware that some devices have a confusing setting that sets the port where SIP requests originate from on the SIP device itself (the bind port). The port specified on this page is NOT this bind port  but the port this service listens on.", 
...
du = s:option(Value, "defaultuser",  translate("User Name"),
         translate("Use (four to five digit) numeric user name if you are connecting normal telephones with ATAs to this system (so they can dial user names)."))
du.datatype = "uciname"

pwd = s:option(Value, "secret", translate("Password"),
               translate("Your password disappears when saved for your protection. It will be changed only when you enter a value different from the saved one."))
pwd.password = true
pwd.rmempty = false
