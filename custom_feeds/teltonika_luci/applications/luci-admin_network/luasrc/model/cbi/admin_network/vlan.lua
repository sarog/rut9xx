...
$Id: vlan.lua 8379 2012-03-19 21:04:15Z jow $
]]--
...
m.uci:foreach("network", "switch",
	function(x)
		...
		s = m:section(NamedSection, x['.name'], "switch",
			switch_title and escapeHTML(translatef("Switch %q (%s)", switch_name, switch_title))
					      or escapeHTML(translatef("Switch %q", switch_name)))
		...
		s = m:section(TypedSection, "switch_vlan",
			switch_title and escapeHTML(translatef("VLANs on %q (%s)", switch_name, switch_title))
						  or escapeHTML(translatef("VLANs on %q", switch_name)))
		...
		local vid = s:option(Value, has_vlan4k or "vlan", translate("VLAN ID"))
